...
export interface ServiceEventCreationAttributes
  extends Omit<ServiceEventAttributes, 'id' | 'createdAt' | 'updatedAt' | 'version' | 'eventStatus'> {
 ...
 */
export class ServiceEvent extends Model<InferAttributes<ServiceEvent>, InferCreationAttributes<ServiceEvent>> {
  // Primary attributes
  ...
  public hasVolumeRecorded(): boolean {
    return this.volumeCollectedCubicYards !== null && this.volumeCollectedCubicYards > 0;
  }
  ...
  public isFullyAssigned(): boolean {
    return this.isAssignedToRoute() && this.hasDriverAssigned() && this.hasVehicleAssigned();
  }
  ...
  public static async findByCustomer(customerId: string): Promise<ServiceEvent[]> {
  ...
  public static async findByDateRange(startDate: Date, endDate: Date): Promise<ServiceEvent[]> {
    return await ServiceEvent.findAll({
      where: {
        scheduledDate: {
  ...
  public static async findByStatus(status: ServiceEventStatus): Promise<ServiceEvent[]> {
    return await ServiceEvent.findAll({
      where: {
        eventStatus: status,
        deletedAt: null,
  ...
  public static async findByType(eventType: ServiceEventType): Promise<ServiceEvent[]> {
    return await ServiceEvent.findAll({
      where: {
        eventType,
        deletedAt: null,
  ...
  public static async getServiceEventStatistics(): Promise<any> {
    ...
    const completionStats = await ServiceEvent.findAll({
      attributes: [
        ...
        [database.fn('COUNT', database.literal('CASE WHEN event_status = \'completed\' THEN 1 END')), 'completedEvents'],
        [database.fn('AVG', database.literal('CASE WHEN actual_start_time IS NOT NULL AND actual_end_time IS NOT NULL THEN EXTRACT(EPOCH FROM (actual_end_time - actual_start_time))/60 END')), 'avgDurationMinutes'],
        [database.fn('SUM', database.col('weight_collected_lbs')), 'totalWeightLbs'],
        [database.fn('SUM', database.col('volume_collected_cubic_yards')), 'totalVolumeCubicYards'],
      ],
      where: { deletedAt: null },
      raw: true,
    });
  ...
  },
  {
    ...
    indexes: [
      ...
      },
      {
        ...
        fields: [database.fn('ST_GeogFromWKB', database.col('service_location'))],
        using: 'GIST',
    ...
    hooks: {
      ...
      beforeUpdate: (serviceEvent: ServiceEvent) => {
        ...
        if (serviceEvent.changed('eventStatus') && serviceEvent.eventStatus === ServiceEventStatus.IN_PROGRESS && !serviceEvent.actualStartTime) {
          serviceEvent.actualStartTime = new Date();
        }
        
        // Auto-set end time when status changes to completed
        if (serviceEvent.changed('eventStatus') && serviceEvent.eventStatus === ServiceEventStatus.COMPLETED && !serviceEvent.actualEndTime) {
    ...
    scopes: {
      ...
      dueToday: {
        where: {
          ...
          eventStatus: {
            [database.Sequelize.Op.in]: [ServiceEventStatus.SCHEDULED, ServiceEventStatus.IN_PROGRESS],
          },
          deletedAt: null,
        },
      },
