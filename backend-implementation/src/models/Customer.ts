...
> {
  ...
  declare service_types: string[];
  declare container_types: string[];
  declare service_frequency: ServiceFrequency;
  declare preferred_day_of_week?: number | null;
  declare preferred_time?: string | null;
  declare special_instructions?: string | null;

  // Billing information
  declare billing_method: CreationOptional<string>;
  declare payment_terms: CreationOptional<PaymentTerms>;
  declare billing_contact_id?: string | null;

  // Service dates
  declare service_start_date?: Date | null;
  declare service_end_date?: Date | null;
  declare next_service_date?: Date | null;

  // Pricing
  declare base_rate?: number | null;
  declare service_rate?: number | null;
  declare container_rate?: number | null;
  declare fuel_surcharge?: number | null;

  // Relationships
  declare account_manager_id?: string | null;
  declare primary_driver_id?: string | null;
  ...
  declare getAccountManager: BelongsToGetAssociationMixin<User>;
  declare getPrimaryDriver: BelongsToGetAssociationMixin<User>;
  declare getBillingContact: BelongsToGetAssociationMixin<User>;
  declare getServiceEvents: HasManyGetAssociationsMixin<ServiceEvent>;
  declare getInvoices: HasManyGetAssociationsMixin<Invoice>;

  // Static associations
  public static associations: {
    organization: Association<Customer, Organization>;
    accountManager: Association<Customer, User>;
    primaryDriver: Association<Customer, User>;
    billingContact: Association<Customer, User>;
    serviceEvents: Association<Customer, ServiceEvent>;
    invoices: Association<Customer, Invoice>;
  };
  ...
  public getTotalRate(): number {
    ...
    if (this.container_rate)
      total += parseFloat(this.container_rate.toString());
  ...
  public async hasActiveServices(): Promise<boolean> {
    return this.canReceiveService();
  }

  public async getServiceHistory(): Promise<ServiceEvent[]> {
    return [];
  }
...
Customer.init(
  {
    ...
    service_types: {
      ...
      defaultValue: [],
      comment:
        "Types of waste management services (trash, recycling, composting, etc.)",
    },
    ...
    billing_method: {
      ...
      validate: {
        isIn: [
          ["monthly_invoice", "automatic_payment", "prepaid", "on_account"],
        ],
      },
    },
  ...
  },
  {
    ...
    hooks: {
      beforeCreate: async (customer: Customer) => {
        ...
        if (customer.service_start_date && customer.service_frequency) {
          customer.next_service_date =
            await customer.calculateNextServiceDate();
        }
      },
      ...
      beforeUpdate: async (customer: Customer) => {
        ...
        if (
          (customer.changed("service_frequency") ||
            customer.changed("service_start_date")) &&
    ...
    scopes: {
      ...
      activeServices: {
        where: {
          status: [CustomerStatus.ACTIVE],
          service_start_date: { [Op.lte]: new Date() },
          [Op.or]: [
            { service_end_date: null },
            { service_end_date: { [Op.gte]: new Date() } },
          ],
          deleted_at: null,
        },
      },
      ...
      dueForService: {
        where: {
          status: CustomerStatus.ACTIVE,
          next_service_date: { [Op.lte]: new Date() },
          deleted_at: null,
        },
      },
...
// Declare static methods for TypeScript
...
Customer.findByStatus = async function (
...
Customer.findDueForService = async function (
  ...
  return await this.scope("dueForService").findAll({
    where: {
      next_service_date: { [Op.lte]: targetDate },
    },
    order: [
...
Customer.findByFrequency = async function (
  frequency: ServiceFrequency,
): Promise<Customer[]> {
  return await this.scope([
    "active",
    { method: ["byFrequency", frequency] },
  ]).findAll({
    order: [["customer_number", "ASC"]],
  });
};
...
Customer.findByAccountManager = async function (
  managerId: string,
): Promise<Customer[]> {
...
export { Customer as CustomerModel, ServiceConfig };
