 ...
 */
export interface OrganizationCreationAttributes extends Omit<OrganizationAttributes,
  'id' | 'createdAt' | 'updatedAt' | 'version' | 'billingCountry' | 'serviceCountry' | 'gdprApplicable' | 'dataRetentionPolicy'
> {
  id?: string;
  billingCountry?: string;
  serviceCountry?: string;
...
> {
  ...
  isInEU(): boolean {
    ...
    return euCountries.includes(this.billingCountry) || 
  ...
  hooks: {
    ...
    afterUpdate: async (org: Organization) => {
      ...
      if (org.changed('status')) {
        console.log(`Organization status changed: ${org.name} to ${org.status}`);
      }
      if (org.changed('type')) {
