...
exports.OrganizationFixtures = void 0;
...
class OrganizationFixtures {
    ...
    async createOrganizations(count, baseData = {}, transaction) {
        const organizations = [];
    ...
    async createWasteManagementCompany(transaction) {
    ...
    async createRecyclingCompany(transaction) {
        return this.createOrganization({
    ...
    async createInactiveOrganization(transaction) {
        return this.createOrganization({
            name: 'Inactive Test Company',
            isActive: false,
        }, transaction);
    }
    async createOrganizationWithPreferences(preferences, transaction) {
        return this.createOrganization({
            name: 'Custom Preferences Org',
            preferences,
        }, transaction);
    }
    getBusinessTypeTestData() {
        return [
            {
                businessType: 'waste_management',
    ...
    async createGeographicTestData(transaction) {
        const locations = [
    ...
    async cleanup(transaction) {
        await Organization_1.Organization.destroy({
            where: {},
            force: true,
            transaction,
        });
...
exports.default = OrganizationFixtures;
